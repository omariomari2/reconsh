#!/bin/bash
#
# common.sh - Common functions and utilities for reconsh
#
...
# Generate cache key from URL and date
cache_key() {
    local url="$1"
    local date_key
    date_key=$(date +%Y%m%d)
...
cached_curl() {
    ...
    # Return cached result if exists and caching is enabled
...
# Rate limiting - random sleep between API calls
rate_limit() {
    local min_delay=${1:-1}
    local max_delay=${2:-3}
...
# JSON helper - safely create JSON object
json_object() {
    local -A pairs
    while [[ $# -gt 0 ]]; do
        local key="$1"
...
# JSON helper - safely create JSON array from stdin
json_array() {
    jq -R . | jq -s .
}
